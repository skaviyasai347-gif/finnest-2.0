...
export const UserLayout: React.FC = () => {
  const { user, signOut, isAdmin, switchDemoPersona } = useAuth();
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="hidden sm:flex items-center bg-emerald-50 border border-emerald-200 rounded-xl px-2.5 py-1 text-xs gap-2">
                <span className="flex items-center gap-1.5 text-emerald-700 font-bold text-[11px] uppercase tracking-wide">
                  <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span>
                  Demo: {user?.role === 'admin' ? 'Superadmin' : 'Owner (' + (user?.full_name?.split(' ')[0] || 'User') + ')'}
                </span>
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold transition-colors shadow-xs"
              >
